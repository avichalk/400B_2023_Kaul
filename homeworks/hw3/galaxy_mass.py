...
def main():
    ...
    with open("tex/out.tex", "w") as f:
        ...
        f.writelines(f"\n\n 4. Baryonic Fraction $f_{{bar}}$ for each galaxy is {', '.join(str(x) for x in [f'{output[i][0]}:{output[i][5]}' for i in range(0, 3)])}. Our baryonic fractions are universally lower than the provided value of 16\%. Here are the True Fraction / Provided Fraction values for the various galaxies: {', '.join(str(x) for x in [f'{output[i][0]}:{output[i][5]/.16}' for i in range(0, 3)])}. This discrepancy might come down to the large amounts of interstellar gas and dust between galaxies; as galaxies are mostly dark matter, their bayonic fraction is significantly lower than the rest of the universe.")
        f.writelines("\n \\end{document}")
