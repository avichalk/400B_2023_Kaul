...
class M33AnalyticOrbit:
    ...
    def __init__(self, filename):
        ...
        self.Vc = 180 ## km/s (circular speed from HW5)
    ...
    def DynamicalFriction(self, Msat, Vc, r, v):
        """ Method that returns the acceleration from dynamical friction.
        
        a = -0.428 * G * Msat * ln(Lambda) / rmag^2 + v/vmag

        Inputs:
            Msat : `float`
                Mass of the satellite (M31) in Msun
            Vc : `float`
                Max circular velocity of the host's Dark Matter Halo in km/s
            r : `array of floats`
                Position vector of the satellite relative to the host in kpc
            v : `array of floats`
                Velocity vector of the satellite relative to the host in km/s

        Outputs:
            DF : `array of floats`
                Deceleration vector owing to Dynamical Friction (km/s)
        ...
        DF = -0.0428 * self.G*Msat*Coulomb/r_mag*v/v_mag
...
if __name__ == '__main__':
    ...
    ax.plot(M33Orbit['t'], M31_M33_R, lw=5, linestyle='--', label="M31-M33 w/ DF")
